...
as
begin
	update Department set DepartmentName=@department_name where DepartmentID=@department_id;
end
...
as
begin
	select * from Department where DepartmentID=@department_id;
end
...
as
begin
	select p.PersonID,p.FirstName,p.LastName,p.JoiningDate,p.Salary,p.DepartmentID,d.DepartmentName,de.DesignationName from Pe as p inner join Department as d on P.DepartmentID=d.DepartmentID inner join Designation as de on p.DesignationID=de.DesignationID;
end
exec person_select
...
as
begin
	select p.FirstName from Department as d inner join Pe as p on d.DepartmentID=p.DepartmentID where d.DepartmentName=@dname;
end
...
as
begin
	select p.PersonID,p.FirstName,p.LastName,p.JoiningDate,p.Salary,p.DepartmentID,d.DepartmentName,de.DesignationName from Pe as p inner join Department as d on P.DepartmentID=d.DepartmentID inner join Designation as de on p.DesignationID=de.DesignationID where p.FirstName=@fname;
end
exec per_name @fname='Rahul'

--4. Create Procedure which displays department wise maximum, minimum & total salaries.
select d.DepartmentName,AVG(p.Salary) as average from Department as d full outer join Pe as p on d.DepartmentID=p.DepartmentID where d.DepartmentName = 'admin' group by d.DepartmentName;
